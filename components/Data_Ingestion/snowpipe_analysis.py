...
def comp_snowpipe_analysis(entry_actions=None):
    ...
    try:
        ...
        with st.expander("Snowpipe Efficiency Analysis", expanded=True):
            ...
            try:
                efficiency_df = st.session_state.session.sql(efficiency_query).to_pandas()
        ...
        with st.expander("Top Credit Consumers & Overhead Analysis", expanded=True):
            ...
            try:
                snowpipe_df = st.session_state.session.sql(snowpipe_query).to_pandas()
        ...
        with st.expander("Ingestion Credit Consumption & Cost Projections", expanded=True):
            ...
            try:
                cost_df = st.session_state.session.sql(cost_projection_query).to_pandas()
...
def _render_eff_cost_chart(df, key_prefix=""):
    ...
    if chart_type == "Bar Chart":
        ...
        st.plotly_chart(fig, use_container_width=True, key=f"{key_prefix}bar")
        st.markdown('<div style="font-size: 11px; color: #666;"><span style="color: #2ca02c;">●</span> Efficient (&lt;0.5) | <span style="color: #ff7f0e;">●</span> Medium (0.5-1) | <span style="color: #d62728;">●</span> Inefficient (&gt;1)</div>', unsafe_allow_html=True)
    elif chart_type == "Pie Chart":
...
def _render_comparison_chart(df, key_prefix=""):
    ...
    if chart_type == "Bar Chart":
        plot_df = chart_df.head(10).sort_values('CREDITS_BURNED', ascending=True)
        fig = go.Figure()
        fig.add_trace(go.Bar(y=plot_df['PIPE_NAME'], x=plot_df['CREDITS_BURNED'], name='Credits Burned', orientation='h', marker_color='#1f77b4', text=[f"{val:.2f}" for val in plot_df['CREDITS_BURNED']], textposition='outside', textfont=dict(size=9)))
        fig.add_trace(go.Bar(y=plot_df['PIPE_NAME'], x=plot_df['GB_LOADED'], name='GB Loaded', orientation='h', marker_color='#2ca02c', text=[f"{val:.2f}" for val in plot_df['GB_LOADED']], textposition='outside', textfont=dict(size=9)))
        ...
        plot_df = chart_df.head(10).sort_values('CREDITS_BURNED', ascending=True)
        fig = go.Figure()
        fig.add_trace(go.Bar(y=plot_df['PIPE_NAME'], x=plot_df['CREDITS_BURNED'], name='Credits Burned', orientation='h', marker_color='#1f77b4'))
        fig.add_trace(go.Bar(y=plot_df['PIPE_NAME'], x=plot_df['GB_LOADED'], name='GB Loaded', orientation='h', marker_color='#2ca02c'))
...
def _render_cost_30days_chart(df, key_prefix=""):
    ...
    if chart_type == "Bar Chart":
        ...
        colors = ['#1f77b4', '#ff7f0e'][:len(plot_df)]
        ...
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#1f77b4", "#ff7f0e"], "series": [{"name": "Credits", "type": "pie", "radius": ["0%", "55%"], "center": ["50%", "40%"], "data": chart_data, "itemStyle": {"borderRadius": 5}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}pie")
    elif chart_type == "Pie - Donut":
        chart_data = [{"value": float(row['LAST_30_DAYS']), "name": f"{row['INGEST_METHOD']} ({row['LAST_30_DAYS']:.1f})"} for _, row in chart_df.iterrows() if pd.notna(row['LAST_30_DAYS'])]
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#1f77b4", "#ff7f0e"], "series": [{"name": "Credits", "type": "pie", "radius": ["40%", "65%"], "center": ["50%", "40%"], "data": chart_data, "itemStyle": {"borderRadius": 8}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}donut")
    else:
        chart_data = [{"value": float(row['LAST_30_DAYS']), "name": f"{row['INGEST_METHOD']} ({row['LAST_30_DAYS']:.1f})"} for _, row in chart_df.iterrows() if pd.notna(row['LAST_30_DAYS'])]
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#1f77b4", "#ff7f0e"], "series": [{"name": "Credits", "type": "pie", "radius": ["20%", "65%"], "center": ["50%", "40%"], "roseType": "area", "data": chart_data, "itemStyle": {"borderRadius": 5}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}rose")
...
def _render_cost_3month_chart(df, key_prefix=""):
    ...
    if chart_type == "Bar Chart":
        ...
        colors = ['#2ca02c', '#d62728'][:len(plot_df)]
        ...
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#2ca02c", "#d62728"], "series": [{"name": "Credits", "type": "pie", "radius": ["0%", "55%"], "center": ["50%", "40%"], "data": chart_data, "itemStyle": {"borderRadius": 5}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}pie")
    elif chart_type == "Pie - Donut":
        chart_data = [{"value": float(row['EST_3_MONTHS']), "name": f"{row['INGEST_METHOD']} ({int(row['EST_3_MONTHS']):,})"} for _, row in chart_df.iterrows() if pd.notna(row['EST_3_MONTHS'])]
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#2ca02c", "#d62728"], "series": [{"name": "Credits", "type": "pie", "radius": ["40%", "65%"], "center": ["50%", "40%"], "data": chart_data, "itemStyle": {"borderRadius": 8}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}donut")
    else:
        chart_data = [{"value": float(row['EST_3_MONTHS']), "name": f"{row['INGEST_METHOD']} ({int(row['EST_3_MONTHS']):,})"} for _, row in chart_df.iterrows() if pd.notna(row['EST_3_MONTHS'])]
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#2ca02c", "#d62728"], "series": [{"name": "Credits", "type": "pie", "radius": ["20%", "65%"], "center": ["50%", "40%"], "roseType": "area", "data": chart_data, "itemStyle": {"borderRadius": 5}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}rose")
...
def _render_cost_6month_chart(df, key_prefix=""):
    ...
    if chart_type == "Bar Chart":
        ...
        colors = ['#9467bd', '#8c564b'][:len(plot_df)]
        ...
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#9467bd", "#8c564b"], "series": [{"name": "Credits", "type": "pie", "radius": ["0%", "55%"], "center": ["50%", "40%"], "data": chart_data, "itemStyle": {"borderRadius": 5}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}pie")
    elif chart_type == "Pie - Donut":
        chart_data = [{"value": float(row['EST_6_MONTHS']), "name": f"{row['INGEST_METHOD']} ({int(row['EST_6_MONTHS']):,})"} for _, row in chart_df.iterrows() if pd.notna(row['EST_6_MONTHS'])]
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#9467bd", "#8c564b"], "series": [{"name": "Credits", "type": "pie", "radius": ["40%", "65%"], "center": ["50%", "40%"], "data": chart_data, "itemStyle": {"borderRadius": 8}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}donut")
    else:
        chart_data = [{"value": float(row['EST_6_MONTHS']), "name": f"{row['INGEST_METHOD']} ({int(row['EST_6_MONTHS']):,})"} for _, row in chart_df.iterrows() if pd.notna(row['EST_6_MONTHS'])]
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#9467bd", "#8c564b"], "series": [{"name": "Credits", "type": "pie", "radius": ["20%", "65%"], "center": ["50%", "40%"], "roseType": "area", "data": chart_data, "itemStyle": {"borderRadius": 5}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}rose")
...
def _render_cost_12month_chart(df, key_prefix=""):
    ...
    if chart_type == "Bar Chart":
        ...
        colors = ['#e377c2', '#7f7f7f'][:len(plot_df)]
        ...
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#e377c2", "#7f7f7f"], "series": [{"name": "Credits", "type": "pie", "radius": ["0%", "55%"], "center": ["50%", "40%"], "data": chart_data, "itemStyle": {"borderRadius": 5}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}pie")
    elif chart_type == "Pie - Donut":
        chart_data = [{"value": float(row['EST_12_MONTHS']), "name": f"{row['INGEST_METHOD']} ({int(row['EST_12_MONTHS']):,})"} for _, row in chart_df.iterrows() if pd.notna(row['EST_12_MONTHS'])]
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#e377c2", "#7f7f7f"], "series": [{"name": "Credits", "type": "pie", "radius": ["40%", "65%"], "center": ["50%", "40%"], "data": chart_data, "itemStyle": {"borderRadius": 8}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}donut")
    else:
        chart_data = [{"value": float(row['EST_12_MONTHS']), "name": f"{row['INGEST_METHOD']} ({int(row['EST_12_MONTHS']):,})"} for _, row in chart_df.iterrows() if pd.notna(row['EST_12_MONTHS'])]
        option = {"legend": {"bottom": "5", "left": "center", "orient": "horizontal", "itemGap": 5, "textStyle": {"fontSize": 10}}, "tooltip": {"trigger": "item", "formatter": "{b}: {d}%"}, "toolbox": {"show": True, "feature": {"dataView": {"show": True, "readOnly": False}, "restore": {"show": True}, "saveAsImage": {"show": True}}}, "color": ["#e377c2", "#7f7f7f"], "series": [{"name": "Credits", "type": "pie", "radius": ["20%", "65%"], "center": ["50%", "40%"], "roseType": "area", "data": chart_data, "itemStyle": {"borderRadius": 5}}]}
        st_echarts(options=option, height="400px", key=f"{key_prefix}rose")
