...
AND (sm.time_travel_bytes + sm.failsafe_bytes) > 0
    ...
    "db_overview_26_expanded_object_count_query": """
    SELECT OBJECT_TYPE, OBJECT_COUNT
    FROM (
        ...
        UNION ALL SELECT 'Permanent Tables', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.TABLES WHERE DELETED IS NULL AND TABLE_TYPE = 'BASE TABLE' AND IS_TRANSIENT = 'NO'
        UNION ALL SELECT 'Transient Tables', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.TABLES WHERE DELETED IS NULL AND TABLE_TYPE = 'BASE TABLE' AND IS_TRANSIENT = 'YES'
        UNION ALL SELECT 'Dynamic Tables', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.TABLES WHERE DELETED IS NULL AND IS_DYNAMIC = 'YES'
        UNION ALL SELECT 'Hybrid Tables', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.TABLES WHERE DELETED IS NULL AND IS_HYBRID = 'YES'
        UNION ALL SELECT 'Iceberg Tables', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.TABLES WHERE DELETED IS NULL AND IS_ICEBERG = 'YES'
        UNION ALL SELECT 'Ext. Tables', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.TABLES WHERE DELETED IS NULL AND TABLE_TYPE = 'EXTERNAL TABLE'
        UNION ALL SELECT 'Views', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.VIEWS WHERE DELETED IS NULL AND IS_SECURE = 'NO'
        UNION ALL SELECT 'Sec. Views', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.VIEWS WHERE DELETED IS NULL AND IS_SECURE = 'YES'
        UNION ALL SELECT 'Mat. Views', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.TABLES WHERE DELETED IS NULL AND TABLE_TYPE = 'MATERIALIZED VIEW'
        UNION ALL SELECT 'Stored Procs', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.PROCEDURES WHERE DELETED IS NULL
        UNION ALL SELECT 'UDFs', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.FUNCTIONS WHERE DELETED IS NULL
        UNION ALL SELECT 'Pipes', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.PIPES WHERE DELETED IS NULL
        UNION ALL SELECT 'Streams', COUNT(DISTINCT TABLE_CATALOG || '|' || TABLE_SCHEMA || '|' || NAME) FROM SNOWFLAKE.ACCOUNT_USAGE.GRANTS_TO_ROLES WHERE GRANTED_ON = 'STREAM' AND DELETED_ON IS NULL
        UNION ALL SELECT 'Tasks', COUNT(DISTINCT TABLE_CATALOG || '|' || TABLE_SCHEMA || '|' || NAME) FROM SNOWFLAKE.ACCOUNT_USAGE.GRANTS_TO_ROLES WHERE GRANTED_ON = 'TASK' AND DELETED_ON IS NULL
        UNION ALL SELECT 'Row Access Policies', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.ROW_ACCESS_POLICIES WHERE DELETED IS NULL
        UNION ALL SELECT 'Masking Policies', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.MASKING_POLICIES WHERE DELETED IS NULL
        ...
        UNION ALL SELECT 'Tags', COUNT(*) FROM SNOWFLAKE.ACCOUNT_USAGE.TAGS WHERE DELETED IS NULL
    )
